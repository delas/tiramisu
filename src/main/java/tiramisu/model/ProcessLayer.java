...
public class ProcessLayer implements ILayer {
    ...
    public void loadFromDFG(List<String> dfg, Map<String, Activity> activities) throws Exception {
        int num_activities = Integer.parseInt(dfg.get(0));
        ...
        for(int i = num_activities + 1; i < dfg.size(); i++) {
            String line = dfg.get(i);
            if (line.contains(">")) {
                ...
                String activity_from_name = dfg.get(from+1);
                String activity_to_name = dfg.get(to+1);
        ...
        for(int i = 0; i < num_activities; i++) {
            String activity_name = dfg.get(i+1);
